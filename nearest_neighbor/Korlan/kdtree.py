...
class KDTree:
    ...
    def _build(self, points, depth):
        ...
        **Implementation Steps:**
        ...
        node.left = self._build(points[:median_index], depth + 1)
        node.right = self._build(points[median_index + 1:], depth + 1)
    ...
    def _knn_search(self, node, query_point, k, neighbors):
        ...
        **Implementation Steps:**
        ...
        # --- YOUR IMPLEMENTATION HERE ---
        distance_sq = self._distance_sq(node.point, query_point)
        
        # Update neighbors heap
        if len(neighbors) < k:
            heapq.heappush(neighbors, (-distance_sq, node.point))
        else:
            if -neighbors[0][0] > distance_sq:
                heapq.heappushpop(neighbors, (-distance_sq, node.point))
        
        splitting_plane_distance_sq = (node.point[node.axis] - query_point[node.axis])**2
        
        # Determine which subtree to explore first
        if query_point[node.axis] < node.point[node.axis]:
            self._knn_search(node.left, query_point, k, neighbors)
            if (len(neighbors) < k or 
                splitting_plane_distance_sq < -neighbors[0][0]):
                self._knn_search(node.right, query_point, k, neighbors)
        else:
            self._knn_search(node.right, query_point, k, neighbors)
            if (len(neighbors) < k or 
                splitting_plane_distance_sq < -neighbors[0][0]):
